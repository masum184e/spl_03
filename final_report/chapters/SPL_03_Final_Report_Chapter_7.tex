...
The preceding sections evaluated four standalone counting approaches -- a pretrained, off-the-shelf person detector (Section~\ref{sec:classic_yolo}), a head detector fine-tuned on the SCUT-Head dataset (Section~\ref{sec:scut_yolo}), and two density-regression networks, MDNN and CSRNet (Sections~\ref{sec:mdnn} and~\ref{sec:csrnet}) -- together with two adaptive hybrid configurations that fuse the fine-tuned detector with each density-regression network in turn (Sections~\ref{sec:hybrid} and~\ref{sec:hybrid_csrnet}). This section draws these results together and interprets the patterns that recur across methods; their broader implications for deployment, limitations, and future directions are addressed in the concluding chapter (Section~\ref{sec:conclusion}).
